<template>
    <div class="card">
        <DataTable v-model:filters="filters" v-model:selection="selectedData" :value="filteredData" paginator :rows="10"
            ...
            <!-- 评分 (仅 volunteer) -->
            <Column v-if="props.role === 'volunteer'" field="rating" header="Rating" style="min-width: 8rem" sortable></Column>
        ...
        <div class="mt-3">
            ...
            <Button label="Send Email to Selected" class="ml-2 p-button-warning" @click="sendEmailsToSelected" :disabled="!selectedData.length" />
        </div>
    </div>
</template>
